...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, tid):
        ...
        #分别对输入特征和上下文增强特征应用 query、key、value 线性变换，生成查询、键和值。
        mixed_query_layer = self.query(hidden_states)
        topic_states = self.query(topic_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
        
        # 调用 transpose_for_scores，调整张量形状，适配多头注意力计算。
        ...
        ### v2 改了这里 
        '''
        计算 topic_layer 和 key_layer 的点积注意力分数。
        对分数进行缩放、归一化（Softmax），并添加 Dropout。
        结合注意力权重和 value_layer，生成中间结果 v_t。
        '''
        ...
        '''
        使用 query_layer 和 topic_layer 再次计算注意力分数。
        将注意力权重与上下文特征 v_t 结合，生成最终的 context_layer。
        '''
